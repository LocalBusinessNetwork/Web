(function (root, factory) {
  ...
  var ReferralWireRouterBase = Backbone.Router.extend({
    ...
    ContactRefWizard : { //this isn't a route it's a declaration on the referral wizard structure that's referenced by several partner views 
          ...
          helpUrl : function() { return rwcore.getLov('ONLINEHELP', 'INVITE_WEB').get('Url') },
                        ...
                        if (!$.hasVal(options.view.radioModels))
                          ...
                          if (updatedList.length > 0){
                            ...
                            if (options.done == "showUpsert"){
                              var v = (_.isUndefined(that.options))?options.view:that.options.view;
                              rwApp.openStandardDialog({view:v});
                            }
...
getReferralInboxHomeTemplate:function(model){
        ...
        if ($.hasVal(model)){
          ...
          if (referralType == "INMAIL_REFERRAL"){
            var referralsubType = model.get("referralSubType");
            if ( referralsubType == 'P2PM') 
              retVal = 'ReferralInboxMessage'; //update this
            else if (referralsubType == 'C2PM')
              retVal = 'ReferralInboxAnnouncement'; //update this
    ...
    getReferralOutboxHomeTemplate:function(model){
        ...
        if ($.hasVal(model)){
          ...
          if (referralType == "INMAIL_REFERRAL"){
            var referralsubType = model.get("referralSubType");
            if ( referralsubType == 'P2PM') 
              retVal = 'ReferralOutboxMessage'; //update this
            else if (referralsubType == 'C2PM')
              retVal = 'ReferralInboxAnnouncement'; //update this
          }
  ...
  getTemplate:function(model){
    ...
    if (_.contains(["EventExpected","EventCheckedIn","EventGuests","phoneEventExpected","phoneEventCheckedIn","phoneEventGuests"],clickRoute)){
      var guestType = model.get('guestType');

      templateName = "AttendeeMemberNoAct";
      privCheck.privilege = "editFull";
      if ($.hasAccess(privCheck)){
        if (guestType == "MEMBER"){templateName = "AttendeeMember";}
        if (guestType == "NEW_MEMBER"){templateName = "AttendeeNewGuest";}
        if (guestType == "FLOATER"){templateName = "AttendeeFloater";}
        ...
        if ($.hasAccess(privCheck)){
            ...
            if (guestType == "FLOATER"){templateName = "AttendeeFloater";}
              ...
              if (guestType == "MEMBER"){templateName = "AttendeeMemberNoAct";}
              if (guestType == "NEW_MEMBER"){templateName = "AttendeeNewGuestNoAct";}
              if (guestType == "FLOATER"){templateName = "AttendeeFloaterNoAct";}
        }
      } 
  ...
  refreshTopView : function(resetContext){
    ...
    if (!$.hasVal(resetContext)){resetContext = false}
     ...
     if (resetContext && (currentRoute.indexOf("/") != -1)){
       ...
       var contextId = currentRoute.substr(currentRoute.lastIndexOf('/')+1);
...
ContactRefWizard_Recommendation : function(){
  ...
  validateCheckIn:function(options){
      ...
      if ( $.hasVal(options.validateTimePlace) && options.validateTimePlace ) { 
          var feetAway = 0 // $.calculateDistance(options) * 5280; // uncomment this if we need to validate the distance 
          var meetingTime = new Date(options.model.get('datetime').$date);
          var minFromMeetingStart = Math.abs(new Date().getTime() - meetingTime)/60000;
          if (feetAway > rwcore.mtgCheckinWindowDistance || minFromMeetingStart > rwcore.mtgCheckinWindowMinutes) {
            $.when(rwcore.showWaitDialog("CheckedInInvalid")).done(function() { rwApp.preventNavigation = false; ; promise.reject(); });
          }
  ...
  validateCheckInGeo:function(options){
  ...
  eventCheckIn : function (options){
    var promise = $.Deferred();
    $.when(rwApp.validateCheckIn(options)).done( function () { 
      options.model.set({status:$.hasVal(options.changeToStatus)?options.changeToStatus:"CHECKEDIN"});
      options.model.save({},{
        success:function(model, textStatus, jqXHR){
            rwApp.preventNavigation = false;               
            if ($.hasVal(options.refresh)){rwApp.refreshTopView();}
            promise.resolve(model);
        },
        error: function(response,status,error) { 
            rwcore.showError(response,status,error); promise.reject();
        }
      });
  ...
  eventSelfCheckIn : function (options){
    var promise = $.Deferred();
    $.when(rwApp.validateCheckIn(options)).done( function () { 
      options.model.set({selfcheckin:true});
      options.model.save({},{
        success : function(model, textStatus, jqXHR){
            if ($.hasVal(options.refresh)){rwApp.refreshTopView();}
            promise.resolve(model);
         ...
         // var searchFields = this.searchFormView.metaApplet.model.attributes.field;
          ...
          var searchFields = searchFormView.metaApplet.model.attributes.field;
            ...
            for (var i = 0; i < searchFields.length; i++){
              ...
              if (($.hasVal(thisVal) || searchOperator == "DISTANCE_FROM" || searchOperator == "WEEKS_AGO") && thisFieldName != "searchName" && thisFieldName != "isDefaultSearch" ){           
                if (searchOperator == "DISTANCE_FROM"){
                  ...
                  if (searchOperator == "EQUALS_ID") {
                      ...
                      searchDefinition[searchDefinition.length]= term;
                        ...
                        if (searchOperator == "LIKE_ID") {
                          var idName = thisField.likeIdField;
                          var idVal = model.get(idName);
                          ...
                          searchDefinition[searchDefinition.length]= term;
                            ...
                            if (searchOperator == "IN" || searchOperator == "NOT_IN") {
                              ...
                              if (searchOperator == "WEEKS_AGO" || searchOperator == "WEEKS_BEFORE"){
                                 var fName = thisField.compareField;
                                 fVal = model.get(fName);
                                 if ($.hasVal(thisVal)){
                                   var term = {fldname:thisFieldName,compareField:fName,value:thisVal}
                                   searchDefinition[searchDefinition.length]= term;
                                 }
                              } else {
   ...
   parseSavedSearch:function(searchModel,searchAppletName){
      ...
      for (var i = 0; i < searchFields.length; i++){
        ...
        if (($.hasVal(thisVal) && !_.isUndefined(thisOperator)) || thisOperator == "DISTANCE_FROM" || thisOperator == "WEEKS_AGO") {
          ...
          if (thisOperator == "DISTANCE_FROM" || thisOperator == "DISTANCE_LENGTH") {
            ...
            if (thisOperator == "EQUALS_ID"){
              ...
              searchRequestTerm.value = searchModel.get(searchRequestTerm.equalsIdField);
                ...
                if (thisOperator == "LIKE_ID"){
                  searchRequestTerm.searchOperator = thisOperator;
                  searchRequestTerm.likeIdField = thisField.likeIdField;
                  searchRequestTerm.value = searchModel.get(searchRequestTerm.likeIdField);
                } else {
                    ...
                    if (thisOperator == "LESS_THAN" || thisOperator == "GREATER_THAN") {
                      ...
                      if (thisOperator == "WEEKS_AGO" || thisOperator == "WEEKS_BEFORE") {
                          //compareField is needed in cases where the same field is used for both a greater than and a less than comparison
                          //thisFieldName = thisField.compareField;
                          //thisVal = searchModel.get(thisFieldName);
                          if (!$.hasVal(thisVal)) {searchRequestTerm = undefined;}
                          else {
                            searchRequestTerm.searchOperator = thisOperator;  
                            searchRequestTerm.value = thisVal;
                            searchRequestTerm.fieldName = thisFieldName;
                            searchRequestTerm.compareField = thisField.compareField;
                          }
        ...
        for ( var i = 0; i < pickMap.length; i++) {
          ...
          if (pickItem["type"] == "pick"){
            ...
            if (toFieldName.indexOf("[") > -1){
              ...
              var pickToMCS = pickItem["pickToMCS"];
              if (!_.isUndefined(pickToMCS)){
                options.form.renderAllMultiCheckSearchControls();
              }
            }
...
if (defaultVal.indexOf("[") > -1){
  ...
  userdata.fetch({
      ...
      var defaultComments = $.getInvitationText(model,toFirstName,toPartyType) //options should include toFirstName, fromFullName, fromBusiness, fromTitle    
            ...
            rwApp.upsertRecord({
              el:'#upsertRecord',
              appletName:(toPartyType=="PARTNER")?"ConnectRequest":"Invitation",
   ...
   newInMail:function(options){
    ...
    userdata.fetch({
         ...
         success : function (model, response, jqXHR) {
          ...
          var fromId = model.get('id');
             ...
             var view = new ReferralWireView.FormView({applet: "Message", 
                 templateHTML : 'UpsertFormNarrow'
             });
             ...
             refrldata.set({
                ...
                subject:"",
                urgency: "NORMAL",
                referralType:'INMAIL_REFERRAL',
                referralSubType:'P2PM',
                fromId:fromId
             });
              
        
             var showConfirmOnSave = 'MessageSentConfirmation';
             rwApp.upsertRecord({
              el:'#upsertRecord',
              appletName: "Message",
              upsertTemplate:'UpsertFormNarrow',
    ...
    newAnnouncement:function(options){
    
        var chapterdata = options.chapter;
        var OrgId = chapterdata.get('id');
   
        var view = new ReferralWireView.FormView({applet: "Announcement", 
           templateHTML : 'UpsertFormNarrow'
        });
        ...
        var refrldata = view.model;
        refrldata.set({
          to_fullName : chapterdata.get('businessName'),
          toId : OrgId,
          subject:"",
          urgency: "NORMAL",
          comments:"",
          referralType:'INMAIL_REFERRAL',
          referralSubType:'C2PM'
        });
        
        var showConfirmOnSave = 'MessageSentConfirmation';
         rwApp.upsertRecord({
          el:'#upsertRecord',
          appletName: "Announcement",
          upsertTemplate:'UpsertFormNarrow',
          upsertModel:refrldata,
          parentView:null,
          showConfirmOnSave:showConfirmOnSave,
   ...
   inMailReply : function ( rfrlId, comments2 ) {
        rwcore.spinOn();
        var segment = "<div class=\"referralFormSectionTitle\"><img src=\"" + rwFB.photoUrl + "\"/><span>&nbsp;"  +  rwFB.fullName + " on " + new Date().toLocaleString() + "</span></div>";
        segment = segment + "<span>" + comments2.val() + "</span>" ;

        var model = new rwcore.StandardModel({module: 'RfrlMgr', id: rfrlId, comments2 : segment, status : 'UNREAD' });
        var promise = model.save({}, {async : true});
        $.when(promise).done( function (data) {
            $("#" + rfrlId + "_msg" ).append( segment );
            comments2.val('');
            rwcore.spinOff();

        }).fail( function() { rwcore.spinOff(); }); 
   },

   getNotifications : function () {
      var thisRequest = $.Deferred();
      var promise = $.ajax({type: "GET", url: "/rwAlerts"});
      
      promise.done( function(model, response, jqXHR) {
            var newNotes = JSON.parse(model.notifications);
            var previousNotesStr = rwcore.cacheout("Notifications");
            if ( newNotes.length > 0) {
              if ( !_.isUndefined(previousNotesStr) ) {
                 var previousNotes = JSON.parse(previousNotesStr);
                 if ( previousNotes != null && previousNotes.length > 0) {
                    newNotes = _.union(newNotes, previousNotes);      
                 }
              }
              rwcore.cachein("Notifications", JSON.stringify(newNotes));
            }
            else {
              if ( !_.isUndefined(previousNotesStr) ) {
                 var previousNotes = JSON.parse(previousNotesStr);
   ...
   showNotifications : function () {
    ...
    if ( !_.isUndefined(previousNotesStr) ) {
       var previousNotes = JSON.parse(previousNotesStr);
       if ( previousNotes != null && previousNotes.length > 0) {
          var msg = "<ul style=\"font-size:10pt\">";  
          for ( var i = 0; i < previousNotes.length ; i++) {
            msg = msg + "<li>" + previousNotes[i].message + "</li>";
          }
          msg = msg + "</ul>";
          $.when(rwcore.FYI(msg, {left:735,top:190})).done(function(){ rwApp.clearNotifications()});
       }
    }
   },

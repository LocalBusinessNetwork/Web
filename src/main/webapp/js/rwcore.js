(function (root, factory) {
  if (typeof exports === 'object') {
	...
	Backbone.rwcore = rwcore = (function(Backbone, _, $){
		...
		rwcore.spinRefCount = 0;
		rwcore.mtgCheckinWindowMinutes = 120; 
		rwcore.mtgCheckinWindowDistance = 400; 
		...
		rwcore.validEmail = function (email){      
			var emailReg = /^[a-zA-Z0-9._%+-]+@[a-zA-Z0-9.-]+\.(?:us|com|net|org|biz|edu|gov|mobi|aero|museum|name|guru|club|email|house|uno|photography|solutions|today|tips|homes|info)$/;
		...
		rwcore.getLov = function(lovType, GlobalVal) {
		...
		rwcore.getLovDisplayVal = function(lovType, GlobalVal) {
			var lov = rwcore.getLov(lovType, GlobalVal);
		    return $.hasVal(lov)? lov.get('DisplayVal'): '';
		};
		...
		rwcore.ErrorView = Backbone.View.extend({
		    ...
		    render:function () {
			    ...
			    ga('send', 'event', 'Error', status, responseText);
	...
	rwcore.HelpView = Backbone.View.extend({
		...
		rwcore.FYI = function(message, position) {
				...
				if ( !_.isUndefined(position)) {
					FYIContainer.css(position);
				}
				else {
					FYIContainer.css({"top":"50%", "left":"60%", "margin-top":"-150px", "margin-left":"-200px"});
				}
	    ...
	    rwcore.showError = function (request,status,error) {
	    	...
	    	ga('send', 'event', 'button', 'click', 'nav-buttons');
	     	...
	     	if ( status && (status.status == 401) ) {
	     		ga('send', 'event', 'Error', status.status);
				window.location = rct.JSP_LOGIN;
	     		return;
	     	}

	     	if( request &&  (request.status == 401 )) {
	     		ga('send', 'event', 'Error', request.status);
	     		window.location = rct.JSP_LOGIN;
	     		return;
			}
